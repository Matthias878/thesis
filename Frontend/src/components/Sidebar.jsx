...
const displayLine = (d, chr) =>
  d
    ? `value=${text(d.value)} · relative=${relPos(d)} · absolute=${absPos(
        d,
        chr
      )}`
    : "—";
...
function setUploadTestState(patch) {
  ...
  const prev = window.__uploadTestState ?? {uploadInProgress: false, lastStartedFile: null, lastCompletedFile: null, lastFailedFile: null, lastError: null, updatedAt: null,};
...
function UploadSection({title, grid, file, setFile, onUpload, disabled, accept, buttonText = "Upload", items, value = "", onSelect, emptyText = "empty", selectTitle = "", getValue = (x) => x, getLabel = (x) => String(x ?? ""), extra, testIdBase, allowClickWithoutFile = false,}) {
...
function ToggleButton({onClick, active, onText, offText, disabled = false, title = "", testId,}) {
  return (
    <button
      data-testid={testId}
      type="button"
  ...
  const sections = [
    {
      ...
      onSelect: (key) =>
        selectValue(key, selectSavedCollection, "preset", "key"),
        ...
        <div style={wrapRow}>
          <ToggleButton
            ...
            onClick={() =>
              runToggle(
                toggleLogoMode,
                "logo button pressed -> toggleLogoMode()"
              )
            }
            active={logoEnabled}
        ...
        <div style={wrapRow}>
          <ToggleButton
            ...
            onClick={() =>
              runToggle(
                toggleMatrixMode,
                "matrix button pressed -> toggleMatrixMode()"
              )
            }
          ...
          <ToggleButton
            ...
            onClick={() =>
              runToggle(
                toggleLineMode,
                "line-mode button pressed -> toggleLineMode()",
                {
                  enabled: canActivateLines,
                  blockedMsg:
                    "line-mode button blocked: canActivateLines=false",
                  reload: true,
                }
              )
            }
    ...
    },
    {
      ...
      title:
        'sequence upload {">name:startpos-endpos\\nSEQ.fasta file"}',
      grid: sectionGrid8,
      file: fastaFile,
    ...
    <>
      ...
      >
        {sections.map((section, i) => (
          <div
            key={section.title}
            data-testid={`${section.testIdBase}-container`}
          >
            <UploadSection {...section} disabled={blockUI} />
        ...
        <div style={hoverBlock} data-testid="hover-info">
          <div data-testid="hover-cell">
            Hover cell:{" "}
            {displayLine(hoveredPosition, currentChromosome)}
          </div>
          <div style={hoverLine} data-testid="last-clicked">
            Last clicked:{" "}
            {displayLine(lastClickedPosition, currentChromosome)}
          </div>
          ...
          <div style={hoverLine} data-testid="absolute-position">
            absolute position:{" "}
            {currentChromosome?.absolutePosition ?? 0}
          </div>
        ...
        <div style={backendRowInStatus} data-testid="api-backend-status">
          ...
          />
          <div
            style={backendTextEllipsis}
            data-testid="api-backend-status-text"
          >
        ...
        <div
          style={backendRowInStatus}
          data-testid="higlass-server-status"
        >
          <span
          ...
          <div
            style={backendTextEllipsis}
            data-testid="higlass-server-status-text"
          >
            {higlassText}
          </div>
        </div>
